...
class TestCLINewCommands:
    ...
    def mock_security_validator(self):
        ...
        with patch("shared.security_validator.validate_user_input") as mock_validate:
            with patch(
                "shared.security_validator.security_validator"
            ) as mock_validator:
    ...
    def test_new_python_project_valid(
        self, cli_runner, mock_secure_cli_manager, mock_security_validator
    ):
    ...
    def test_new_python_project_invalid_name(self, cli_runner, mock_security_validator):
        ...
        mock_security_validator["validate_user_input"].return_value = (
            False,
    ...
    def test_new_python_project_current_dir(
        self, cli_runner, mock_secure_cli_manager, mock_security_validator
    ):
    ...
    def test_new_javascript_project_valid(
        self, cli_runner, mock_secure_cli_manager, mock_security_validator
    ):
    ...
    def test_new_javascript_project_with_type(
        self, cli_runner, mock_secure_cli_manager, mock_security_validator
    ):
        ...
        result = cli_runner.invoke(
            womm, ["new", "javascript", "test-react-project", "--type", "react"]
        )
    ...
    def test_new_detect_project_current_dir(
        self, cli_runner, mock_secure_cli_manager, mock_security_validator
    ):
...
class TestCLINewErrorHandling:
    ...
    def test_new_project_script_validation_failure(
        self, cli_runner, mock_security_validator
    ):
        ...
        mock_security_validator[
            "security_validator"
        ].validate_script_execution.return_value = (False, "Script validation failed")
    ...
    def test_new_project_command_execution_failure(
        self, cli_runner, mock_security_validator
    ):
    ...
    def test_new_project_security_validation_failure(
        self, cli_runner, mock_security_validator
    ):
...
class TestCLINewArgumentValidation:
    ...
    def test_javascript_project_type_validation(
        self, cli_runner, mock_secure_cli_manager, mock_security_validator
    ):
        ...
        for project_type in valid_types:
            result = cli_runner.invoke(
                womm, ["new", "javascript", "test-project", "--type", project_type]
            )
            assert result.exit_code == 0

        # Test invalid type
        result = cli_runner.invoke(
            womm, ["new", "javascript", "test-project", "--type", "invalid"]
        )
        assert result.exit_code != 0
