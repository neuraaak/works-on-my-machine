...
class SecureCLIManager:
            ...
            try:
                ...
                if self.verbose:
                    if secure_result.success:
                        print(
                            f"✅ {display_desc} - SUCCESS ({secure_result.execution_time:.2f}s)"
                        )
                        if secure_result.stdout.strip():
                            print(secure_result.stdout.strip())
                    else:
                        print(
                            f"❌ {display_desc} - FAILED (code: {secure_result.returncode})"
                        )
                ...
                if self.verbose:
                    print(
                        f"⏱️ Timeout after {run_timeout}s (attempt {attempt + 1}/{self.max_retries})"
                    )
                ...
                if self.verbose:
                    ...
                    print(
                        f"❌ Error during {desc_text} (attempt {attempt + 1}/{self.max_retries}): {e}"
                    )
                if attempt < self.max_retries - 1:
                    time.sleep(self.retry_delay)
    ...
    def _execute_command(
        ...
        **kwargs,
        ...
        # Execute command (security validated in calling method)
...
def get_tool_version_secure(
    tool: str, version_flag: str = "--version"
) -> Optional[str]:
