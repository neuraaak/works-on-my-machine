...
class LintManager:
    ...
    def run_python_lint(
        self, target_path: Optional[Path] = None, fix: bool = False
    ) -> LintSummary:
        ...
        if fix:
            ...
            # Check mode
            summary.tool_results.append(self._run_ruff_check(target_dirs, target_dir))
    ...
    def _run_ruff_check(self, target_dirs: List[str], cwd: Path) -> LintResult:
        """Run ruff check."""
        result = run_command(
        ...
        return LintResult(
            ...
            tool_name="ruff",
            message="Ruff style check completed",
    ...
    def _run_ruff_fix(self, target_dirs: List[str], cwd: Path) -> LintResult:
        ...
        return LintResult(
            success=result.success,
            tool_name="ruff",
            message="Ruff style fix completed",
            files_checked=len(target_dirs),
        )
    ...
    def _run_ruff_format(self, target_dirs: List[str], cwd: Path) -> LintResult:
        ...
        return LintResult(
            success=result.success,
            tool_name="ruff",
            message="Ruff formatting completed",
            files_checked=len(target_dirs),
        )
    ...
    def _run_black_check(self, target_dirs: List[str], cwd: Path) -> LintResult:
        ...
        return LintResult(
            success=result.success,
            tool_name="black",
            message="Black format check completed",
            files_checked=len(target_dirs),
        )
    ...
    def _run_black_format(self, target_dirs: List[str], cwd: Path) -> LintResult:
        ...
        return LintResult(
            success=result.success,
            tool_name="black",
            message="Black formatting completed",
            files_checked=len(target_dirs),
        )
    ...
    def _run_isort_check(self, target_dirs: List[str], cwd: Path) -> LintResult:
        ...
        return LintResult(
            success=result.success,
            tool_name="isort",
            message="Isort import check completed",
            files_checked=len(target_dirs),
        )
    ...
    def _run_isort_fix(self, target_dirs: List[str], cwd: Path) -> LintResult:
        ...
        return LintResult(
            success=result.success,
            tool_name="isort",
            message="Isort import fix completed",
            files_checked=len(target_dirs),
        )
    ...
    def _run_bandit_check(self, target_dirs: List[str], cwd: Path) -> LintResult:
        ...
        return LintResult(
            success=result.success,
            tool_name="bandit",
            message="Bandit security check completed",
            files_checked=len(target_dirs),
        )
