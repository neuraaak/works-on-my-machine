...
def run_spellcheck(path: Path, fix_mode: bool = False) -> bool:
    ...
    # Validation de sécurité préalable
    try:
    ...
    try:
        ...
        result = run_command(  # noqa: S604
            cmd, f"Spell check - {path.name}", shell=True
        )
        if result.success:
            return True

        # Fallback: essayer avec npx si cspell direct échoue
        ...
        # Valider la commande npx si possible
        if use_security_validation:
            is_valid, error = security_validator.validate_command(cmd)
            if not is_valid:
