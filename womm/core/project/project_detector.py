...
def interactive_project_selection() -> str:
    ...
    print("🎯 What type of project do you want to create?")
    print()

    for i, (_ptype, description) in enumerate(available_types, 1):
        print(f"{i}. {description}")

    print(f"{len(available_types) + 1}. Automatic detection")
    print()
        ...
        try:
            ...
            if choice == str(len(available_types) + 1):
                ...
                if confidence > 0:
                    ...
                    print("⚠️  No type detected, using Python type by default")
                    return "python"

            choice_idx = int(choice) - 1
            if 0 <= choice_idx < len(available_types):
                selected_type = available_types[choice_idx][0]
                print(f"✅ Selected: {available_types[choice_idx][1]}")
...
def launch_project_setup(
    ...
    if project_type in ["python"]:
        ...
        print(f"❌ Project type '{project_type}' not supported")
        return 1
...
Examples:
    ...
    if args.detect:
        detector = ProjectDetector()
        detected_type, confidence = detector.detect_project_type()
        print(f"Detected type: {detected_type} (confidence: {confidence})")
        return 0

    # Determine project type
    if args.type:
        project_type = args.type
        print(f"🎯 Specified type: {project_type}")
        ...
        if confidence > 0:
            ...
            print("⚠️  No type detected, manual selection required")
