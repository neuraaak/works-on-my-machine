...
class BackupManager:
    ...
    def list_backup_files(self, include_metadata: bool = True) -> List[Dict]:
        ...
        Raises:
            BackupError: If backup directory access fails
            ContextUtilityError: For unexpected errors
        """
        backup_files = []
    ...
    def load_backup_file(self, filepath: str) -> Tuple[bool, Dict, str]:
        ...
        Raises:
            ...
            BackupError: If backup loading fails
            ContextUtilityError: For unexpected errors
        """
        try:
            # Input validation
            ...
            if not backup_path.exists():
                raise BackupError("load", filepath, "Backup file not found")

            try:
                with open(backup_path, encoding="utf-8") as f:
                    data = json.load(f)
            except json.JSONDecodeError as e:
                raise BackupError("load", filepath, f"Invalid JSON format: {e}") from e
            except (PermissionError, OSError) as e:
                raise BackupError(
    ...
    def cleanup_old_backups(
        ...
        Raises:
            ...
            BackupError: If cleanup operations fail
            ContextUtilityError: For unexpected errors
        """
        try:
            # Input validation
    ...
    def get_backup_info(self, filepath: str) -> Dict:
        ...
        Raises:
            ValidationError: If filepath is None or empty
            BackupError: If backup info retrieval fails
            ContextUtilityError: For unexpected errors
        """
        try:
            # Input validation
    ...
    def create_backup_copy(
        ...
        Raises:
            ...
            BackupError: If backup copy operation fails
            ContextUtilityError: For unexpected errors
        """
        try:
            # Input validation
            ...
            if not source_path.exists():
                raise BackupError("copy", source_filepath, "Source file not found")
    ...
    def merge_backups(
        ...
        Raises:
            ValidationError: If parameters are invalid
            BackupError: If merge operation fails
            ContextUtilityError: For unexpected errors
        """
        try:
            # Input validation
