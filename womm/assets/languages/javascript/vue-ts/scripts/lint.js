#!/usr/bin/env node
// ///////////////////////////////////////////////////////////////
// VUE-TS LINT - Vue (TypeScript) Project Linting Script
// ///////////////////////////////////////////////////////////////

/**
 * Vue (TypeScript) project linting script.
 *
 * This script automates code quality checks:
 * - ESLint for style and error detection (with Vue + TypeScript support)
...
/**
 * Detect Vue TypeScript directories while excluding sensitive files.
 *
...
function detectProjectDirs(basePath = null) {
  ...
  try {
    ...
    // Search for directories with TypeScript/Vue files
    for (const item of items) {
      if (
        ...
        // Check if it contains TypeScript/Vue files
        let hasTsFiles = false;
        try {
          const tsFiles = fs
            .readdirSync(path.join(currentDir, item.name))
            .filter(
              (f) => f.endsWith('.ts') || f.endsWith('.vue')
            );
          if (tsFiles.length > 0) {
            hasTsFiles = true;
          }
    ...
    // Fallback: analyze current directory if it contains safe TS/Vue files
...
function main(targetPath = null) {
  console.log('🚀 Vue TypeScript linting script started!');
  const targetDir = targetPath ? path.resolve(targetPath) : process.cwd();

  console.log('💚 Vue (TypeScript) Project - Linting Script');
  ...
  if (!fs.existsSync(packageJsonPath)) {
    console.log('❌ No package.json found. This may not be a Vue TypeScript project.');
    return 1;
  }
  ...
  if (targetDirs.length === 0) {
    console.log('❌ No TypeScript/Vue folders found');
    return 1;
  }
  ...
  // 2. Check linting with ESLint (Vue + TypeScript rules)
  console.log('🔍 Checking code with ESLint (Vue + TypeScript)...');
  ...
  // 3. Check formatting with Prettier (Vue support)
...
function fixCode(targetPath = null) {
  ...
  console.log('🔧 Vue (TypeScript) Project - Automatic code fixing');
  console.log('='.repeat(50));
  console.log(`📂 Target directory: ${targetDir}`);
