#!/usr/bin/env node
// ///////////////////////////////////////////////////////////////
// REACT LINT - React (JavaScript) Project Linting Script
// ///////////////////////////////////////////////////////////////

/**
 * React (JavaScript) project linting script.
 *
 * This script automates code quality checks:
 * - ESLint for style and error detection (with React support)
...
/**
 * Detect React JavaScript directories while excluding sensitive files.
 *
...
function detectProjectDirs(basePath = null) {
  ...
  try {
    ...
    // Search for directories with JavaScript/JSX files
    for (const item of items) {
      if (
        ...
        // Check if it contains JavaScript/JSX files
...
function main(targetPath = null) {
  console.log('🚀 React linting script started!');
  const targetDir = targetPath ? path.resolve(targetPath) : process.cwd();

  console.log('⚛️  React (JavaScript) Project - Linting Script');
  ...
  if (!fs.existsSync(packageJsonPath)) {
    console.log('❌ No package.json found. This may not be a React project.');
    return 1;
  }
  ...
  if (targetDirs.length === 0) {
    console.log('❌ No JavaScript/JSX folders found');
    return 1;
  }
  ...
  // 1. Check linting with ESLint (React rules)
  console.log('🔍 Checking code with ESLint (React)...');
  ...
  // 2. Check formatting with Prettier (JSX support)
...
function fixCode(targetPath = null) {
  ...
  console.log('🔧 React (JavaScript) Project - Automatic code fixing');
  console.log('='.repeat(50));
  console.log(`📂 Target directory: ${targetDir}`);
