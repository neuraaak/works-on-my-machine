...
def configure_project_options(project_type: str) -> dict[str, str | bool | int]:
    ...
    """
    ezprinter.info(f"⚙️  Configuring {project_type} project options")
    ezprinter.info("=" * 50)
...
def _configure_common_options() -> dict[str, Any]:
    ...
    if INQUIRERPY_AVAILABLE:
        ...
        ezprinter.info("License options:")
        ezprinter.info("1. MIT License")
        ezprinter.info("2. Apache License 2.0")
        ezprinter.info("3. GNU General Public License v3.0")
        ezprinter.info("4. BSD 3-Clause License")
        ezprinter.info("5. Custom license")
...
def _configure_python_options() -> dict[str, Any]:
    ...
    if INQUIRERPY_AVAILABLE:
        ...
        options["python_version"] = inquirer.select(  # pyright: ignore[reportPrivateImportUsage]
        ...
        # Development tools
        options["include_dev_tools"] = inquirer.confirm(  # pyright: ignore[reportPrivateImportUsage]
        ...
        if options["include_dev_tools"]:
            ...
            selected_tools = inquirer.checkbox(  # pyright: ignore[reportPrivateImportUsage]
        ...
        # Project structure
        options["create_src_layout"] = inquirer.confirm(  # pyright: ignore[reportPrivateImportUsage]
            message="Use src/ layout (recommended for packages)?",
            default=True,
        ).execute()

        options["include_docs"] = inquirer.confirm(  # pyright: ignore[reportPrivateImportUsage]
        ...
        # Fallback to simple input
        ezprinter.info("Python version options:")
        versions = ["3.8", "3.9", "3.10", "3.11", "3.12"]
        for i, version in enumerate(versions, 1):
            ezprinter.info(f"{i}. Python {version}")
        ...
        if options["include_dev_tools"]:
            ezprinter.info("Development tools:")
            tools = ["black", "flake8", "isort", "mypy", "pytest", "pre-commit"]
            for i, tool in enumerate(tools, 1):
                ezprinter.info(f"{i}. {tool}")
            tools_input = input(
...
def _configure_javascript_options(project_type: str) -> dict[str, Any]:
    ...
    if INQUIRERPY_AVAILABLE:
        ...
        if project_type == "javascript":
            ...
            options["js_project_type"] = inquirer.select(  # pyright: ignore[reportPrivateImportUsage]
        ...
        options["package_manager"] = inquirer.select(  # pyright: ignore[reportPrivateImportUsage]
        ...
        if project_type in ["react", "vue"]:
            options["use_typescript"] = inquirer.confirm(  # pyright: ignore[reportPrivateImportUsage]
        ...
        if project_type == "react":
            options["use_jsx"] = inquirer.confirm(  # pyright: ignore[reportPrivateImportUsage]
                message="Use JSX syntax?",
                default=True,
            ).execute()

        elif project_type == "vue":
            options["vue_version"] = inquirer.select(  # pyright: ignore[reportPrivateImportUsage]
        ...
        ezprinter.info("Package managers:")
        managers = ["npm", "yarn", "pnpm"]
        for i, manager in enumerate(managers, 1):
            ezprinter.info(f"{i}. {manager}")
        ...
        if project_type == "react":
            ...
            ezprinter.info("Vue.js versions:")
            ezprinter.info("1. Vue 3 (Composition API)")
            ezprinter.info("2. Vue 2 (Options API)")
...
__all__ = ["configure_project_options"]
